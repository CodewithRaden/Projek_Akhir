...
def monitoring():
    ...
    if user_filter:
        user_data = supabase.table("users").select("id").eq("username", user_filter).execute()
        if user_data.data:
    ...
    if locker_number_filter:
        locker_data = supabase.table("lockers").select("id").eq("locker_number", locker_number_filter).execute()
        if locker_data.data:
